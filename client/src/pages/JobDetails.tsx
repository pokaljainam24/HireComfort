...
const JobDetails: React.FC = () => {
  ...
  const [rejectedAtRound, setRejectedAtRound] = useState<number | null>(null);
  ...
  const getCompanyLogo = (): string => {
    ...
    const cleanLogo = logo.replace(/\\/g, "/").replace(/^\/+/, "");
  ...
  const getSalary = (): string => {
    if (typeof job?.salaryRange === "number" && job.salaryRange > 0) {
  ...
  const getExperience = (): string => {
    if (job?.exp !== undefined && job?.exp !== null) {
      return job.exp ? `${job.exp} Years` : "Entry Level";
    }
  ...
  const getInterviewRounds = (): InterviewRound[] => {
    const totalRounds = Number((job as any)?.noOfRounds || 0);
  ...
  const getRoundsPassed = (status: string | null): number => {
    if (!status) {
      return 0;
    }
     ...
     */
    const match = status.match(/^Round\s+(\d+)$/i);
     ...
     */
    if (status === "Hired") {
      return Number((job as any)?.noOfRounds || 0);
    }
  ...
  const loadApplicationStatus = async () => {
    ...
    try {
      ...
      const appRes = await fetch(`${API_BASE_URL}/api/job_application_master`);
      ...
      if (existingApp) {
        setApplicationStatus(existingApp.applicationStatus || "Viewed");

        setRejectedAtRound(existingApp.rejectedAtRound ?? null);
      } else {
        /*
        ...
        setRejectedAtRound(null);
      }
    } catch (appError) {
      console.error("Failed to load application status:", appError);
    }
  };
  ...
  const handleApplyNow = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        createdBy: user.userName || user.username || "applicant",
      };
      ...
      if (response.ok) {
        ...
        setApplicationStatus(
          resData.jobApplication?.applicationStatus || "Viewed",
        );
        ...
        Swal.fire({
          ...
          text: resData.message || "Failed to submit job application.",
          position: "top-end",
          toast: true,
          showConfirmButton: false,
          timer: 3000,
  ...
  useEffect(() => {
    ...
    const loadJobDetails = async () => {
      ...
      try {
        ...
        const jobResponse: any = await jobApi.getOne(id);
         ...
         */
        const data: any =
          jobResponse?.jobMaster || jobResponse?.data || jobResponse;

        console.log("FULL JOB RESPONSE:", jobResponse);

        console.log("NO OF ROUNDS:", data?.noOfRounds);

        console.log("COMPANY DATA:", data?.companyId);

        console.log("RAW COMPANY LOGO:", data?.companyId?.companyLogo);
        ...
        try {
          ...
          const stateId = data.state || data.stateId;

          const countryId = data.country || data.countryId;
          ...
          if (cityId || stateId || countryId) {
            ...
            if (stateId) {
              const state = allStates.find(
                (x) => String(x.id) === String(stateId),
              );

              if (state) {
                locParts.push(state.name);
              }
            }
            ...
            if (countryId) {
              const country = allCountries.find(
                (x) => String(x.id) === String(countryId),
              );

              if (country) {
                locParts.push(country.name);
              }
            }
          }
          ...
          console.error("Failed to parse location:", locationErr);
      ...
      <main className="main">
        <section className="section-box-2">
          <div className="container py-5 text-center">
            ...
            <p className="mt-15 color-text-paragraph">Loading job details...</p>
          </div>
        </section>
      </main>
    );
  ...
  if (error || !job) {
    return (
      <main className="main">
        <section className="section-box-2">
          <div className="container py-5 text-center">
            <h3 className="color-text-paragraph">{error || "Job not found"}</h3>

            <Link to="/jobs" className="btn btn-default mt-20">
              Back to Jobs
            </Link>
          </div>
        </section>
  ...
  const industry =
    jobData.industry || jobData.industryName || jobData.industryId?.name;

  const jobLevel = jobData.jobLevel || jobData.jobLevelName || jobData.level;

  const deadline = jobData.deadline || jobData.applicationDeadline;

  const updatedDate = jobData.updatedAt;

  const companyAddress = jobData.companyId?.location || locationStr;

  const companyPhone =
    jobData.companyId?.phone || jobData.companyId?.contactNumber;

  const companyEmail = jobData.companyId?.email;

  const companyAbout = job?.companyId?.aboutCompany;

  const mapLocation = companyAddress || locationStr || "";
   ...
   */
  const rejectedRoundFromStatus = applicationStatus?.match(
    /^Round\s+(\d+)\s+Rejected$/i,
  );

  const actualRejectedRound =
    rejectedAtRound ??
    (rejectedRoundFromStatus ? Number(rejectedRoundFromStatus[1]) : null);

  const isRejected =
    applicationStatus === "Rejected" || !!rejectedRoundFromStatus;

  const roundsPassed = isRejected
    ? Math.max(0, (actualRejectedRound ?? 1) - 1)
    : getRoundsPassed(applicationStatus);
          ...
          <div className="row mt-10 align-items-center">
            <div className="col-lg-8 col-md-12">
              ...
              <div className="mt-0 mb-15">
                {job.jobType && (
                  <span className="card-briefcase">{job.jobType}</span>
                )}

                {getPostedDate() && (
                  <span className="card-time">{getPostedDate()}</span>
                )}
              </div>
          ...
          <div className="row">
            ...
            <div className="col-lg-8 col-md-12 col-sm-12 col-12">
                  ...
                  {industry && (
                    <div className="col-md-6 d-flex">
                      ...
                      <div className="sidebar-text-info ml-10">
                        ...
                        <strong className="small-heading">{industry}</strong>
                      </div>
                    </div>
                  )}
                  ...
                  {jobLevel && (
                    <div className="col-md-6 d-flex mt-sm-15">
                      ...
                      <div className="sidebar-text-info ml-10">
                        ...
                        <strong className="small-heading">{jobLevel}</strong>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="row mt-25">
                  <div className="col-md-6 d-flex mt-sm-15">
                    ...
                    <div className="sidebar-text-info ml-10">
                      ...
                      <strong className="small-heading">{getSalary()}</strong>
                    </div>
                  </div>
                  ...
                  {locationStr && (
                    <div className="col-md-6 d-flex mt-sm-15">
                      ...
                      <div className="sidebar-text-info ml-10">
                        ...
                        <strong className="small-heading">{locationStr}</strong>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="content-single mt-30">
                  ...
                  <div className="d-flex flex-wrap gap-2 mt-20">
                    {job.skills.map((skill, index) => (
                      <span key={index} className="btn btn-grey-small">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              <div className="content-single mt-50">
                ...
                <p>
                  {(companyAbout && (
                    <div dangerouslySetInnerHTML={{ __html: companyAbout }} />
                  )) ||
              ...
              {applicationStatus && interviewRounds.length > 0 && (
                <div
                ...
                >
                  <InterviewRounds
                     ...
                     */
                    rejectedRound={isRejected ? actualRejectedRound : null}
                    editable={false}
                  />
                </div>
              )}
                ...
                <div className="row align-items-center">
                  ...
                  <div className="col-md-7 text-lg-end social-share">
                    ...
                    <a
                      ...
                      onClick={(e) => e.preventDefault()}
                    >
                      <img src={shareTw} alt="Twitter" width={35} height={35} />
                    </a>
              ...
              {applyMsg && (
                <div className="alert alert-success mt-20">{applyMsg}</div>
              )}

              {applyError && (
                <div className="alert alert-danger mt-20">{applyError}</div>
              )}
            </div>
            ...
            <div className="col-lg-4 col-md-12 col-sm-12 col-12 pl-40 pl-lg-15 mt-lg-30">
              ...
              <div className="sidebar-border">
                <div className="sidebar-heading">
                  <div className="avatar-sidebar">
                    ...
                    <div className="sidebar-info">
                      ...
                      {jobData.openJobs !== undefined && (
                        <span className="link-underline mt-15">
                          {jobData.openJobs} Open Jobs
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="sidebar-list-job">
                  ...
                  <ul className="ul-disc mt-20">
                    ...
                    {companyPhone && <li>Phone: {companyPhone}</li>}

                    {companyEmail && <li>Email: {companyEmail}</li>}
                  </ul>
                </div>
              </div>
              ...
              <div className="sidebar-border mt-30">
                <h6 className="f-18">Job Summary</h6>
                ...
                <div className="sidebar-list-job">
                  <ul className="ul-disc">
                    ...
                    <li>
                      <strong>Salary:</strong> {getSalary()}
                    </li>

                    <li>
                      <strong>Experience:</strong> {getExperience()}
                    </li>

                    {job.status && (
                      <li>
                        <strong>Status:</strong>{" "}
                        <span className="color-green">{job.status}</span>
                      </li>
                    )}
                  </ul>
                </div>
